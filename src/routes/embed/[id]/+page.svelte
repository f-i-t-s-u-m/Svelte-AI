<script lang="ts">
  ...
  beforeUpdate(() => {
		if (chatView) {
			const scrollableDistance = chatView.scrollHeight - chatView.offsetHeight;
			autoscroll = chatView.scrollTop > scrollableDistance - 20;
		}
	});
	...
	const questions = [
		...
		'Are there any current promotions or special offers available?'
	];
...
<main>
	<button
		class="absolute bottom-4 right-4 inline-flex items-center justify-center text-sm font-medium disabled:pointer-events-none disabled:opacity-50 border rounded-full w-16 h-16 bg-black hover:bg-gray-700 m-0 cursor-pointer border-gray-200 bg-none p-0 normal-case leading-5 hover:text-gray-900"
		type="button"
		...
		>
			...
			<div class="flex flex-col space-y-1.5 p-6 py-4 border-b">
				...
				<p class="text-sm text-[#6b7280] leading-3">
					{data?.description ?? 'Lets talk about ' + data.title}
				</p>
			</div>

			<!-- Chat Container -->
			<div class="pr-4 h-full overflow-y-auto p-6 relative" bind:this={chatView} style="max-height: calc(100% - 150px);">
				...
				{#each $messages as message}
					{#if message.role == 'assistant'}
						...
						<div class="flex gap-3 my-4 text-gray-600 text-sm flex-1">
							...
							<p class="leading-relaxed">
								<span class="block font-bold text-gray-700">AI </span>{message.content}
							</p>
						</div>
					{:else}
						<!--  User Chat Message -->
						<div class="flex gap-3 my-4 text-gray-600 text-sm flex-1">
							...
							<p class="leading-relaxed">
								<span class="block font-bold text-gray-700">You </span>{message.content}
							</p>
						</div>
					{/if}
				{/each}
		...
		<div class="fixed bottom-24 right-4 max-w-96">
			<ol reversed class="flex flex-col gap-2 items-end px-2 text-right">
				{#each questions as question}
					<li class="p-2 rounded-md w-96 bg-gray-100 float-end border">
						<button on:click={() => handleMessageClick(question)}>{question}</button>
					</li>
				{/each}
			</ol>
		</div>
